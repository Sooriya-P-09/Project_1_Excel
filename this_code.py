...
def merge_files(file1, file2, file3, selected_columns):
    df1 = pd.read_excel(file1, engine='openpyxl') if file1.name.endswith('.xlsx') else \
      pd.read_excel(file1, engine='xlrd') if file1.name.endswith('.xls') else \
      pd.read_csv(file1)

    df2 = pd.read_excel(file2, engine='openpyxl') if file2.name.endswith('.xlsx') else \
      pd.read_excel(file2, engine='xlrd') if file2.name.endswith('.xls') else \
      pd.read_csv(file2)

    df3 = pd.read_excel(file3, engine='openpyxl') if file3.name.endswith('.xlsx') else \
      pd.read_excel(file3, engine='xlrd') if file3.name.endswith('.xls') else \
      pd.read_csv(file3)
